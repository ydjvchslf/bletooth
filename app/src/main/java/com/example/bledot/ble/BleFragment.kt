...
//        checkCrnState()
        ...
        bleViewModel.mConnectionState.observe(viewLifecycleOwner) { connectState ->
            if (connectState == 2) {
                if(bleViewModel.BLE_STATE.value == BleState.CONNECTED) {
                    return@observe
                }
                ...
                BleDebugLog.d(logTag, "BLE_STATE : ${bleViewModel.BLE_STATE.value}")
                if(bleViewModel.BLE_STATE.value == BleState.NOT_SCANNED) {
                    return@observe
                }
                val list = BleDevice.fromHashMapList(bleViewModel.mScannedSensorList) as ArrayList // 스캔된 데이터 리스트
